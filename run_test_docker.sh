#!/usr/bin/env bash
# Docker 内用（またはホストで直接実行する用）。nrc_ws を source して test + 比較。
# 通常は run_test.sh から呼ばれる（ホストでビルド → Docker でこのスクリプト）。
...
if [[ -z "${TP_TEST_COMMIT:-}" ]] && [[ -n "$NRC_WS_DEVEL" ]] && command -v git &>/dev/null; then
...
exec ./evaluation_docker.sh --side test --compare
