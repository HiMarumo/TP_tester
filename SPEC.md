...
対象範囲:
- baseline 記録
- baseline 用 `input.bag` 生成（`trajectory_predictor_sim` 利用時）
...
- `framed input bag`: `baseline_results/<scene>/input.bag`。`/tp_sim/input_frame` を1フレーム1メッセージで格納した bag。
...
- `node`:
  ...
  - （`trajectory_predictor_sim` 利用時）`create_tp_sim_input` 実行用 pkg
  - viewer 実行用 pkg/node
...
- `baseline_results/<rel>/input.bag`（`trajectory_predictor_sim` 利用時のみ）
...
## 5.0 `create_tp_sim_input` + `trajectory_predictor_sim` 入力仕様（timestamp基準）
対象:
- `projects/nrc_ws/src/nrc_wm/nrc_wm_svcs/src/trajectory_predictor/create_tp_sim_input_node.cpp`
- `projects/nrc_ws/src/nrc_wm/nrc_wm_svcs/src/trajectory_predictor/trajectory_predictor_io_sim.cpp`

### 5.0.1 全体構成
- `trajectory_predictor_sim` を使う場合は 2段構成:
  1. `create_tp_sim_input`: raw topic 群を時刻整列して `/tp_sim/input_frame` を publish。
  2. `trajectory_predictor_sim`: `/tp_sim/input_frame` のみを購読して予測実行。
- `trajectory_predictor_sim` の live/raw topic 直接購読モードは持たない。

### 5.0.2 `create_tp_sim_input` の入力収集
- 駆動トリガは `TrackedObjectSet2`（既定 `/target_tracker/tracked_object_set2`）。
- `TrackedObjectSet2` 受信ごとに queue へ enqueue。
...
### 5.0.3 `create_tp_sim_input` の同期待機と fallback
- 1ms timer で queue 先頭を処理。
- 先頭 `TrackedObjectSet2` の時刻を `t_obj` とし、stamped topic 全てで
  `latest_stamp > t_obj + 1e-4` を満たすまで待機。
- 待機上限は 0.5秒。超過時は timeout fallback として処理を継続。
- 各 stamped topic は `<= t_obj` の最新1件を採用し、採用済み古いデータをバッファから破棄する。

### 5.0.4 `TrajectoryPredictorSimInputFrame` 生成
- 1 `TrackedObjectSet2` につき 1 `TrajectoryPredictorSimInputFrame` を生成し `/tp_sim/input_frame` へ publish。
- frame の `tracked_object_set2` は queue 先頭メッセージをそのまま格納。
- 各補助入力は `has_*` と payload で格納（値が無い場合は `has_* = false`）。
- `frame.header` は `tracked_object_set2.header` をコピー。

### 5.0.5 `trajectory_predictor_sim` の入力と出力stamp
- `trajectory_predictor_sim` は `framed_input_topic_str`（既定 `/tp_sim/input_frame`）のみ購読。
- 各 frame 受信で `has_*` が立っている入力だけを適用し、`TP.Exe()` を実行。
- 出力 `header.stamp` は `frame.tracked_object_set2.header.stamp` を使用。
- その cycle で publish する `record_topics` 9トピックは同一 `header` を共有する。
...
- `trajectory_predictor_sim` 利用時（現行標準）:
  1. `create_tp_sim_input` を起動。
  2. `test_bags/<rel>/*.bag` を `rosbag play --clock` で同時再生し、`/tp_sim/input_frame` を `baseline_results/<rel>/input.bag` に記録。
  3. `trajectory_predictor_sim` は `input.bag` のみを再生入力として受ける。
- `trajectory_predictor`（非sim）利用時:
  - 従来通り `test_bags/<rel>/*.bag` を直接再生入力とする。
- 出力記録時は、共通で以下を行う:
  1. `rosbag record` 開始（結果記録）
  2. `/clock` pre-roll 2秒
  3. 再生対象bagを `rosbag play --clock` で再生
  4. `wait_publish_count` 優先、失敗時 `wait_publish_silence` で停止判定
...
- `trajectory_predictor_sim` 利用時の stamp 決定規則は 5.0（framed入力）を参照。

### 5.1.3 baseline の出力
- `trajectory_predictor_sim` 利用時は scene ごとに `input.bag` を先に生成。
...
- observed 生成で読む入力bagは実行モードで異なる:
  - baseline + `trajectory_predictor_sim`: `baseline_results/<rel>/input.bag`
  - test + `trajectory_predictor_sim`: `baseline_results/<rel>/input.bag`（baselineで生成済みの同一 framed 入力を再利用）
  - 非sim: `test_bags/<rel>/*.bag`
...
- 優先順:
  1. 呼び出し側が指定した `preferred_source_topic`
     - sim時: `/tp_sim/input_frame`
     - 非sim時: `settings.observed_source_topic`（設定されていれば）
  2. 固定候補順:
     - `/tp_sim/input_frame`
...
- `trajectory_predictor_sim` 利用時は baseline/test とも同一 `input.bag`（`/tp_sim/input_frame`）由来で observed を作るため、時刻軸の入力差は原理的に発生しない。
- 非simで raw bag から observed を作る場合は、`settings.observed_source_topic` を trajectory_predictor 入力 topicに合わせることを推奨。
- 差異は生成アルゴリズムのみ:
...
### 5.3.1 test の入力の集め方
- `trajectory_predictor_sim` 利用時は `baseline_results/<rel>/input.bag` を再生入力として使う。
  - baseline と test で同一 framed 入力を使うことで、入力差起因の揺れを排除する。
- `trajectory_predictor`（非sim）利用時は `test_bags/<rel>/*.bag` を直接再生入力とする。
...
1. 同一コミットでも runごとに stamp 集合差が出ることがある。
   - 背景: `create_tp_sim_input` は ObjectSet2 基準同期だが、stamped topic の到着遅延時は 0.5秒で timeout fallback して処理継続する。
   - そのため境界時刻付近では「どの入力が `<= t_obj` 最新として採用されるか」が再生実行ごとに揺れる場合がある。
   - ただし test は baseline で生成した同一 `input.bag` を再利用するため、同一 run 内の baseline/test 比較は入力差の影響を受けない。
