...
- validation評価生成: `/validation/eval/<side>/<threshold>/<group>/<status>`
...
- `baseline_results/<rel>/<threshold>_<group>_baseline_<status>.bag`
...
- `test_results/<rel>/<threshold>_<group>_test_<status>.bag`
...
- observed妥当性評価:
  - `validation.baseline.strict.{ok,total,rate,detail}`
  - `validation.baseline.loose.{ok,total,rate,detail}`
  - `validation.test.strict.{ok,total,rate,detail}`
  - `validation.test.loose.{ok,total,rate,detail}`
...
9. `baseline_results/<rel>/<threshold>_<group>_baseline_<status>.bag`（存在時）
10. `test_results/<rel>/<threshold>_<group>_test_<status>.bag`（存在時）
...
- 9: Diff other path
- 10: Observed
- 11: Baseline strict validation along
- 12: Baseline strict validation opposite
- 13: Baseline strict validation crossing
- 14: Baseline strict validation other
- 15: Baseline loose validation along
- 16: Baseline loose validation opposite
- 17: Baseline loose validation crossing
- 18: Baseline loose validation other
- 19: Test strict validation along
- 20: Test strict validation opposite
- 21: Test strict validation crossing
- 22: Test strict validation other
- 23: Test loose validation along
- 24: Test loose validation opposite
- 25: Test loose validation crossing
- 26: Test loose validation other
...
- 11..18: validation baseline stream（`/validation/eval/baseline/{strict|loose}/{group}/{status}/WM/tracked_object_set_with_prediction`）
- 19..26: validation test stream（`/validation/eval/test/{strict|loose}/{group}/{status}/WM/tracked_object_set_with_prediction`）
...
### 11..26 Validation observed vs prediction
- strict/loose と side(baseline/test) と group(along/opposite/crossing/other)の組み合わせを表示する。
- path色:
...
## 11.2 閾値
- strict:
  - observed yaw 基準の前後誤差: ±2.0m
  - observed yaw 基準の左右誤差: ±0.5m
- loose:
  - observed yaw 基準の前後誤差: ±5.0m
  - observed yaw 基準の左右誤差: ±1.0m
- other group は observed 軌跡の先頭 3.0 秒のみ評価対象とする。

## 11.3 判定手順（object_id単位）
- groupごと（along/opposite/crossing/other）に observed 軌跡と予測 path を比較し、全評価stepが閾値内の path を候補とする。
- 候補 path には二次差分コストを与え、`object_id` ごとに group横断で最小コストの候補を `optimal` とする。
...
## 11.4 出力bag
- side, threshold, group, flag ごとに bag を出力する。
- ファイル名:
  - `<threshold>_<group>_<side>_optimal.bag`
  - `<threshold>_<group>_<side>_ignore.bag`
  - `<threshold>_<group>_<side>_fail.bag`
  - `<threshold>_<group>_<side>_observed_ok.bag`
  - `<threshold>_<group>_<side>_observed_ng.bag`
- 各 bag の記録 topic:
  - `/validation/eval/<side>/<threshold>/<group>/<status>/WM/tracked_object_set_with_prediction`
  - `<status>` は `optimal / ignore / fail / observed_ok / observed_ng`。

## 11.5 success rate
- object_id を frame単位で集計し、`OK率 = ok_count / total_count` を算出する。
- strict/loose × baseline/test の4指標を `comparison.json` の `validation` キーに保存する:
  - `validation.baseline.strict.{ok,total,rate,detail}`
  - `validation.baseline.loose.{ok,total,rate,detail}`
  - `validation.test.strict.{ok,total,rate,detail}`
  - `validation.test.loose.{ok,total,rate,detail}`
...
- 行は `strict success rate`, `loose success rate` の2行、列は `Baseline`, `Test`。
