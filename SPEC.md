...
9. `baseline_results/<rel>/collision_judgement_baseline.bag`（存在時）
10. `test_results/<rel>/collision_judgement_test.bag`（存在時）
11. `baseline_results/<rel>/validation_baseline.bag`（存在時）
12. `test_results/<rel>/validation_test.bag`（存在時）
...
- `common.bag` / `diff_*.bag` / `validation_*.bag` / `collision_judgement_*.bag` は scene ごとに1ファイルのみ生成する。
