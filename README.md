...
初回セットアップから baseline/test 実行までの最短手順です。
...
### 3. 任意で native evaluator をビルドする

validation / collision を高速化したい場合のみ実行します。
不要なら飛ばして構いません。

```bash
cd ~/TP_tester
./build_native_eval.sh
```
...
offline 実行をやり直さず、既存の `result_*.bag` / `observed_*.bag` に対して評価だけ回します。
...
補足:
- `./run_baseline.sh` / `./run_test.sh` は、ホスト側の `nrc_ws` で `catkin_make --pkg nrc_wm_svcs` を実行してから Docker 内処理を流します
- `./evaluation.sh` は既存の結果に対して評価と比較だけを再実行します
- `native/libtp_tester_native_eval.so` が存在すれば native evaluator を自動使用します
- Dockerfile を変えていない限り、`./build_native_eval.sh` のために Docker image を rebuild する必要はありません
...
#### native evaluator (`libtp_tester_native_eval.so`) について

- validation / collision の重い計算は、任意で C++ shared library (`native/libtp_tester_native_eval.so`) を使えます。
- この `.so` は **Docker image には焼き込まず、`~/TP_tester/native/` に配置して読み込む**構成です。
- そのため、**Docker image の rebuild は不要**です。`Dockerfile` を変えていない限り `./build.sh` / `docker compose build` をやり直す必要はありません。
- ただし **現在の Docker image には C++ compiler (`g++`) を入れていない**ため、`.so` のビルドはホスト側で行ってください。
...
- 生成物:
  - `native/libtp_tester_native_eval.so`
- `.so` が存在すれば `run_baseline.sh` / `run_test.sh` 実行時に自動で native evaluator を使います。
- `.so` が無い場合は従来どおり Python 実装に自動フォールバックします。
- 別パスの `.so` を使いたい場合は `TP_TESTER_NATIVE_EVAL_SO=/path/to/libtp_tester_native_eval.so` を指定してください。
...
```bash
cd ~/TP_tester
./build_native_eval.sh   # 任意。validation / collision を高速化したい場合のみ
./run_baseline.sh
```
...
- 実行後、内部で `./evaluation_docker.sh --side baseline --skip-compare` を呼び、baseline 側の observed 評価を更新します。
...
```bash
cd ~/TP_tester
./build_native_eval.sh   # 任意。未ビルドなら Python 実装のまま動作
./run_test.sh
```
...
- 実行後、内部で `./evaluation_docker.sh --side test --compare` を呼び、test 側評価と baseline/test 比較・集約を更新します。
